...
def dl(ctx, profile, cdm, *_, **__):
    ...
    cdm = CdmPr.from_device(device) if "sl3000" in device.get_name() or "sl2000" in device.get_name() else Cdm(device)
    if "sl3000" in device.get_name() or "sl2000" in device.get_name(): 
        cdm.device = device
                ...
                if not track.key:
                    ...
                    try:
                        if "common_privacy_cert" in list(ctx.obj.cdm.__dict__.keys()):
                            ...
                            wrm_header = PSSH(track.pssh).get_wrm_headers(downgrade_to_v4=True)
                            challenge = ctx.obj.cdm.get_license_challenge(session_id, wrm_header)
                            
                            if service_name in ['DisneyPlus', 'Max', 'NowTV', 'SkyShowtime']:
                                ctx.obj.cdm.parse_license(
                                        session_id,
                                        service.license(
                                            challenge=challenge,
