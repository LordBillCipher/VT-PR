...
prd_path = "hisense_smarttv_hu50a6100uw_sl3000.prd"
...
with Path(prd_path).open(mode="rb") as f:
	data = f.read()
	prd_header = DeviceStructs.header.parse(data)
	if prd_header.version == 3:
		device = Device(**DeviceStructs.v3.parse(data))


		device_v2 = _Device(
			group_certificate=device.group_certificate.dumps(),
			encryption_key=device.encryption_key.dumps(),
			signing_key=device.signing_key.dumps()
		)
		prd_bin = device_v2.dumps()

		out_dir = Path.cwd()
		out_path = out_dir / f"{device.get_name()}_v2.prd"
