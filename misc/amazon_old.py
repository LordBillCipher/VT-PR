...
class Amazon(BaseService):
    ...
    @click.option("-am", "--amanifest", default="H265",
                  ...
                  help="Manifest to use for audio. Defaults to H265 if the video manifest.xml is missing 640k audio.")
    ...
    def __init__(self, ctx, title, bitrate: str, cdn: str, vquality: str, single: bool,
        ...
        if self.vquality_source != ParameterSource.COMMANDLINE:
            if 0 < self.quality <= 576 and self.range == "SDR":
                self.log.info(" + Setting manifest.xml quality to SD")
                self.vquality = "SD"

            if self.quality > 1080:
                self.log.info(" + Setting manifest.xml quality to UHD to be able to get 2160p video track")
                self.vquality = "UHD"
    ...
    def get_tracks(self, title: Title) -> Tracks:
        ...
        encoding_version = default_url_set["urls"]["manifest.xml"]["encodingVersion"]
        ...
        if chosen_manifest["streamingTechnology"] == "DASH":
            ...
            raise self.log.exit(f"Unsupported manifest.xml type: {chosen_manifest['streamingTechnology']}")
        ...
        if need_separate_audio and not self.atmos:
            ...
            self.log.info(f"Getting audio from {manifest_type} manifest.xml for potential higher bitrate or better codec")
            ...
            if not audio_manifest:
                ...
                self.log.info(" + Downloading HEVC manifest.xml")
        ...
        if need_uhd_audio and (self.config.get("device") or {}).get(self.profile, None):
            self.log.info("Getting audio from UHD manifest.xml for potential higher bitrate or better codec")
            ...
            try:
                if self.cdm.device.type == LocalDevice.Types.CHROME and self.quality < 2160:
                    self.log.info(f" + Switching to device to get UHD manifest.xml")
            ...
            if not uhd_audio_manifest:
                ...
                self.log.info(" + Downloading UHD manifest.xml")
                ...
                try:
                    ...
                    # replace the audio tracks with DV manifest.xml version if atmos is present
    ...
    def choose_manifest(self, manifest: dict, cdn=None):
        """Get manifest.xml URL for the title based on CDN weight (or specified CDN)."""
        if cdn:
            cdn = cdn.lower()
    ...
    def get_manifest(
        ...
        # if "rightsException" in manifest.xml["returnedTitleRendition"]["selectedEntitlement"]:
        #     if ignore_errors:
        #         return {}
    ...
    def get_original_language(manifest):
        """Get a title's original language from manifest.xml data."""
        try:
            return next(
    ...
    def clean_mpd_url(mpd_url, optimise=True):
        """Clean up an Amazon MPD manifest.xml url."""
        if optimise:
    ...
    def get_best_quality(self, title):
        """
        Choose the best quality manifest.xml from CBR / CVBR
        """
        ...
        for bitrate in bitrates:
            ...
            if not manifest:
                self.log.warning(f"Skipping {bitrate} manifest.xml due to error")
                continue
                
            # return three empty objects if a rightsException error exists to correlate to manifest.xml, chosen_manifest, tracks
            if "rightsException" in manifest["returnedTitleRendition"]["selectedEntitlement"]:
        ...
        if len(self.bitrate.split('+')) > 1:
            ...
            self.log.info("Selected video manifest.xml bitrate: %s", best_quality['bitrate'])

        return best_quality['manifest.xml'], best_quality['chosen_manifest'], best_quality['tracks']
