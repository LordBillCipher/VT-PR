    ...
    def get_titles(self):
        ...
        if data["pageContext"]["subPageType"] == "Movie":
            ...
            if not data["titleContent"]:
                ...
                if len(titles) == 25:
                    page_count = 1
                    token = next((quote(item.get('token')) for item in data.get('episodeList', {}).get('actions', {}).get('pagination', []) if item.get('tokenType') == 'NextPage'), None)
                    while True:
                        page_count += 1
                        res = self.session.get(
                        ...
                        token = next((quote.parse.quote(item.get('token')) for item in data.get('actions', {}).get('pagination', []) if item.get('tokenType') == 'NextPage'), None)
                        if not token:
                            break
            else:
                cards = [
