...
def parse_manifest_ism(url):
    ...
    #Below generates WV PSSH, specifies protection scheme as CENC
    ...
    for stream_info in stream_indices if isinstance(stream_indices, list) else [stream_indices]:
        ...
        if type_info in {'video', 'audio'}:
            ...
            for quality_level in quality_levels:
                ...
                #init = requests.get(init_url).text
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Script for parsing Smooth Streaming manifest.xml URLs.')
